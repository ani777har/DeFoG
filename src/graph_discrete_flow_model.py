...
class GraphDiscreteFlowModel(HyperparameterSearchMixin, pl.LightningModule):
    def __init__(
        self,
        cfg,
        dataset_infos,
    ...
    def on_test_epoch_end(self) -> None:
        ...
        if getattr(self.cfg.sample, "measure_decoding_error", False):
            ...
            # Tag the probe rows with this run's config, exactly as the search
            # path does, so probe_report.py can tell several runs apart.
